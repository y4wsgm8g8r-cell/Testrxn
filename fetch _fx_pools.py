...
MARKETS_QUERY = """
query FxMarkets {
  markets(first: 300, where: { chainId_in: [1] }) {
    items {
      uniqueKey
      loanAsset { symbol }
...
VAULTS_QUERY = """
query AllVaults {
  vaults(first: 300, where: { chainId_in: [1] }) {
    items {
      address
      name
      asset { symbol }
...
def graphql(query: str) -> dict:
    resp = requests.post(MORPHO_GRAPHQL_URL, json={"query": query}, timeout=30)
    if not resp.ok:
...
def collect_direct_market() -> dict | None:
    """Pull every fx-related market and pick the fxSAVE/USDC one directly.

    We deliberately don't hardcode a market's uniqueKey: those can change,
    and a stale ID would silently return nothing. Filtering the live list by
    collateral symbol is more robust.
    """
    data = graphql(MARKETS_QUERY)
...
def collect_rockawayx_vault() -> dict | None:
    """Match purely on curator name -- the vault name won't necessarily
    contain the literal strings FXUSD/FXSAVE/FXN, so don't filter on that.
    """
