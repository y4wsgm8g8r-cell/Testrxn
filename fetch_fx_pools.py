...
def _pack_v2_vault(v: dict) -> dict:
    ...
    rewards_raw = sum(float(r.get("supplyApr") or 0) for r in (v.get("rewards") or []))
    base_pct = round((float(apy_raw) - rewards_raw) * 100, 2)
    rewards_pct = sum(r["apr_pct"] for r in rewards)
    return {
        "address": v["address"],
        "name": v.get("name") or "",
        "asset": (v.get("asset") or {}).get("symbol"),
        "total_assets_usd": round(v.get("totalAssetsUsd") or 0, 2),
        "net_apy_pct": round(base_pct + rewards_pct, 2),
        "rewards": rewards,
...
def render_mcap_chart(history: list[dict] | None) -> str:
    ...
    <div class="peg-chart">
      ...
      <p class="peg-value"><span class="live-dot"></span>{_fmt_compact_usd(current)}</p>
...
def main() -> None:
    ...
    html = render_html(market, vault, defillama, fxusd_mcap, fxsave_mcap, fxsave_apy, pendle_rockawayx, fxusd_peg_history, hydrex_pools, fxusd_mcap_history)
