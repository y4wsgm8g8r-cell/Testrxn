...
def collect_yieldz_pools() -> list[dict]:
    """Two fxSAVE/USDC Morpho markets surfaced on Yieldz leverage."""
    out = []
...
def render_yieldz_card(m: dict, fxsave_apy: float | None = None) -> str:
    lltv = (m.get("lltv_pct") or 0) / 100
    borrow = m.get("borrow_apy_pct")
    max_apy = None
    if fxsave_apy is not None and borrow is not None and 0 < lltv < 0.99:
        max_apy = round((fxsave_apy - borrow * lltv) / (1 - lltv), 2)
    max_html = f"{max_apy}%" if max_apy is not None else "—"
    sid = (m.get("id") or "")[:10]
    return f"""
    <a class="card" href="{YIELDZ_URL}" target="_blank" rel="noopener" data-yz-id="{m.get('id') or ''}" data-yz-lltv="{m.get('lltv_pct') or 0}">
      <p class="card-title">{m['collateral']} / {m['loan']}</p>
      ...
      <div class="stat-row">
        ...
        <div class="stat">
          <p class="stat-label">Leverage</p>
          <p class="stat-value apy">Open</p>
        </div>
      </div>
    </a>
    """
    ...
    (function() {{
      ...
      function refreshAll() {{
        ...
        fetch('https://api.morpho.org/graphql', {{
          ...
          headers: {{ 'Content-Type': 'application/json' }},
          body: JSON.stringify({{
            query: 'query($ids: [String!]!) {{ markets(where: {{ uniqueKey_in: $ids, chainId_in: [1] }}) {{ items {{ marketId lltv state {{ netSupplyApy netBorrowApy liquidityAssetsUsd utilization }} }} }} }}',
            variables: {{ ids: yzIds }}
          }})
        }})
          .then(function(r) {{ return r.json(); }})
            ...
            if (!items || !items.length) {{
              return Promise.all(yzIds.map(function(id) {{
                return fetch('https://api.morpho.org/graphql', {{
                  ...
                  headers: {{ 'Content-Type': 'application/json' }},
                  body: JSON.stringify({{
                    query: 'query($marketId: String!, $chainId: Int!) {{ marketById(marketId: $marketId, chainId: $chainId) {{ marketId lltv state {{ netSupplyApy netBorrowApy liquidityAssetsUsd utilization }} }} }}',
        ...
        fetch('https://yields.llama.fi/pools')
          ...
          .then(function(payload) {{
            ...
            for (var i = 0; i < rows.length; i++) {{
              if (rows[i].pool === '{FXSAVE_DEFILLAMA_POOL_ID}' && typeof rows[i].apy === 'number') {{
                window.__fxsaveApy = rows[i].apy;
                document.querySelectorAll('[data-yz-id]').forEach(function(card) {{
                  var sid = (card.getAttribute('data-yz-id') || '').slice(0, 10);
                  var apyEl = document.getElementById('yz-' + sid + '-apy');
                  var borrowEl = card.getAttribute('data-yz-borrow');
                  var lltv = parseFloat(card.getAttribute('data-yz-lltv') || '0') / 100;
                  var borrow = parseFloat(borrowEl);
                  if (apyEl && lltv > 0 && lltv < 0.99 && !isNaN(borrow)) {{
                    apyEl.textContent = ((window.__fxsaveApy - borrow * lltv) / (1 - lltv)).toFixed(2) + '%';
                  }}
                }});
                return;
              }}
      ...
      function applyYzMarket(m) {{
        ...
        if (card && typeof st.netBorrowApy === 'number') card.setAttribute('data-yz-borrow', String(st.netBorrowApy * 100));
