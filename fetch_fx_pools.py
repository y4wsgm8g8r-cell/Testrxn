...
def collect_fxsave_total_balance() -> float | None:
    """Reads totalAssets() directly from the fxSAVE vault contract on-chain,
    via a public Ethereum RPC -- standard ERC-4626 view function, no API
    key needed. This should closely match the "Total Balance" figure shown
    on the official f(x) Protocol app (fx.aladdin.club/v2/fxsave).
    """
    try:
        ...
        payload = {
            "jsonrpc": "2.0",
            "method": "eth_call",
            "params": [{"to": FXSAVE_CONTRACT, "data": selector}, "latest"],
            "id": 1,
        }
        resp = requests.post(ETH_RPC_URL, json=payload, timeout=20)
        ...
        print(f"[warn] could not read fxSAVE totalAssets on-chain: {exc}", file=sys.stderr)
        return None
...
def render_html(
    ...
    fxsave_onchain_balance: float | None = None,
) -> str:
    ...
    fxsave_onchain_val = fxsave_onchain_balance or 0
  ...
  <div class="wrap">
    <div class="mcap-row">
      ...
      <div class="mcap-card">
        ...
        <p class="mcap-price">Total Balance (on-chain): ${fxsave_onchain_val:,.0f}</p>
      </div>
    </div>
...
def main() -> None:
    ...
    fxsave_onchain_balance = collect_fxsave_total_balance()
        ...
        json.dump(
            {
                ...
                "fxsave_onchain_total_balance_usd": fxsave_onchain_balance,
            },
            f,
            indent=2,
            ensure_ascii=False,
    ...
    html = render_html(market, vault, defillama, fxusd_mcap, fxsave_mcap, fxsave_onchain_balance)
