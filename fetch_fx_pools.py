...
def collect_stablecoin_mcaps() -> dict[str, float]:
    """Both fxUSD and fxSAVE are tracked as pegged assets on DeFiLlama, so
    one request gets us both market caps."""
    out: dict[str, float] = {}
    try:
        ...
        for s in payload.get("peggedAssets", []):
            ...
            if symbol in ("FXUSD", "FXSAVE"):
                mcap = (s.get("circulating") or {}).get("peggedUSD")
                out[symbol] = round(mcap or 0, 2)
    except Exception as exc:  # noqa: BLE001
        print(f"[warn] could not fetch stablecoin market caps: {exc}", file=sys.stderr)
    return out
...
body {
  ...
  background: linear-gradient(180deg, #10261a 0%, #0d1f15 100%);
  color: var(--text);
  font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif;
}
.wrap { max-width: 900px; margin: 0 auto; }
h1 { font-size: 1.7rem; font-weight: 600; margin-bottom: 0.25rem; display: flex; align-items: center; gap: 10px; }
.live-dot {
...
.section-title {
  font-size: 1.35rem; font-weight: 700; margin: 2.5rem 0 0.85rem;
        ...
        fetch('https://stablecoins.llama.fi/stablecoins?includePrices=true')
          ...
          .then(function(data) {{
            ...
            for (var i = 0; i < list.length; i++) {{
              var symbol = (list[i].symbol || '').toUpperCase();
              if (symbol === 'FXUSD') {{
                setValue('fxusd-mcap', (list[i].circulating || {{}}).peggedUSD);
              }} else if (symbol === 'FXSAVE') {{
                setValue('fxsave-mcap', (list[i].circulating || {{}}).peggedUSD);
              }}
            }}
          }})
          .catch(function() {{ /* keep last known values on failure */ }});
      }}
...
def main() -> None:
    ...
    mcaps = collect_stablecoin_mcaps()
    fxusd_mcap = mcaps.get("FXUSD")
    fxsave_mcap = mcaps.get("FXSAVE")
