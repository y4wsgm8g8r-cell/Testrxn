...
SCRIPT_VERSION = "v9-debug-proxy-en-orden"
...
class ExchangeClient:
    ...
    def _init_real_client(self):
        ...
        #   0 = EOA directa (MetaMask/Rabby firmando y operando
        #       directamente, sin wallet intermedia)
        #   1 = Email/Magic (wallet integrada de Polymarket)
        #   2 = Wallet externa vía el proxy VIEJO de Polymarket (tipo
        #       Gnosis Safe) -- este fue el default original que usamos,
        #       pero encontramos evidencia de que Polymarket migró a un
        #       esquema más nuevo.
        #   3 = "Deposit wallet" (EIP-7702) -- CONFIRMADO como el tipo
        #       recomendado actualmente para usuarios NUEVOS creando
        #       credenciales de API por primera vez (justo tu caso).
        #       Cambiamos el default de 2 a 3 después de que el intento
        #       con 2 diera "Could not create api key".
        # Si esto tampoco funciona, ajustá con la variable de entorno
        # POLYMARKET_SIGNATURE_TYPE probando 0, 1 o 2.
    ...
    def place_aggressive_order(self, token_id: str, price_cents: float, size: float) -> str:
        ...
        with _proxy_scope():
            print(f"[DEBUG] Proxy activo al colocar la orden: "
                  f"HTTPS_PROXY={'sí' if os.environ.get('HTTPS_PROXY') else 'NO'} "
                  f"(esperado: sí, si PROXY_URL está configurada)")
