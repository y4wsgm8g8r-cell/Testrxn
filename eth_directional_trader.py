...
SCRIPT_VERSION = "v2-trader-real-5m-con-proxy"
...
# ---------------- Proxy para el trading real ----------------
# Polymarket bloquea activamente la posibilidad de OPERAR (no solo ver
# mercados) desde ciertas jurisdicciones -- si el datacenter de Railway
# cae en una de esas regiones, la API de trading (clob.polymarket.com)
# podría rechazar las órdenes aunque la conexión de solo lectura (ya
# probada y funcionando) no tuvo ese problema.
#
# En vez de pasar el proxy manualmente a cada llamada (y arriesgarme a
# no cubrir alguna que la librería de trading hace por dentro sin que
# yo la vea), seteamos las variables de entorno estándar HTTP_PROXY /
# HTTPS_PROXY a nivel de todo el proceso -- tanto `requests` como la
# gran mayoría de librerías HTTP en Python (incluida py-clob-client-v2,
# que usa requests por dentro) las respetan automáticamente sin
# necesitar configuración adicional.
_PROXY_URL = os.environ.get("PROXY_URL")
if _PROXY_URL:
    os.environ["HTTP_PROXY"] = _PROXY_URL
    os.environ["HTTPS_PROXY"] = _PROXY_URL
    print(f"[DEBUG] PROXY_URL configurada -- todas las llamadas HTTP "
          f"(Gamma API, trading) van a pasar por el proxy.")
else:
    print("[DEBUG] PROXY_URL no configurada -- llamadas HTTP directas, "
          "sin proxy. Si el trading real falla por bloqueo geográfico, "
          "configurá PROXY_URL en Railway.")
...
async def run():
    ...
    await asyncio.gather(btc_price_loop(), eth_odds_loop(), market_refresh_loop())
