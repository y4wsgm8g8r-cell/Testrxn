...
SCRIPT_VERSION = "v18-saca-sigterm-por-riesgo-de-duplicar-ordenes"

# ---------------- NOTA sobre el manejo de SIGTERM que SACAMOS ----------------
# Antes interceptábamos SIGTERM para forzar un reinicio automático en el
# plan gratis de Railway. Pero encontramos un problema serio: cuando
# hacés un REDEPLOY normal, Railway también manda SIGTERM al contenedor
# VIEJO para apagarlo -- y ese mismo truco hacía que el viejo se
# "resucitara" en vez de apagarse, quedando corriendo EN PARALELO con
# el nuevo. Con el bot operando en real, dos instancias vivas al mismo
# tiempo podían llegar a duplicar órdenes (las dos detectan el mismo
# salto de BTC y cada una manda su propia orden). Sacamos el truco:
# preferimos que a veces haya que redesplegar a mano en el plan
# gratis, antes que arriesgar duplicar operaciones con dinero real.
...
def find_active_eth_5m_market() -> dict:
    ...
    for m in markets:
        ...
        if "eth" in slug.lower() and "5m" in slug.lower() and "15m" not in slug.lower():
            ...
            # RESPALDO: si "endDate" no vino en la respuesta de la API
            # (encontramos que esto puede pasar y deja end_ts en None
            # para siempre, congelando la detección de rotación de
            # mercado), el propio slug trae un timestamp al final
            # (ej. eth-updown-5m-1786816800 -> confirmado que es un
            # Unix timestamp real, alineado a bloques de 5 minutos).
            # Lo usamos como respaldo, no reemplazamos endDate si sí vino.
            if end_ts is None:
                import re
                match = re.search(r"-(\d{10})$", slug)
                if match:
                    end_ts = float(match.group(1))
                    print(f"[DEBUG] endDate no vino de la API para {slug} -- "
                          f"usando el timestamp del propio slug como respaldo: {end_ts}")
...
class ExchangeClient:
    ...
    def place_aggressive_order(self, token_id: str, order_size_usd: float) -> str:
        """
        Manda la orden al servicio Rust -- AHORA usando exactamente el
        mismo patrón que la línea de tu bot Flask que sí funciona:
            send_order_with_retry(token_id, 0.0, order_size, "market")
        Precio = 0.0 (Polymarket ejecuta al mejor precio disponible,
        no hace falta calcularlo) y size = el monto en dólares directo,
        sin ninguna división -- lo que estaba mal antes era justamente
        que yo calculaba una cantidad de contratos a mano, cuando el
        campo "size" para una orden de mercado ya espera el monto en
        dólares tal cual.
        """
        if SIMULATION_MODE:
            ...
            print(f"[SIM] Orden colocada: token={token_id[:16]}... "
                  f"monto=${order_size_usd:.2f} (id={order_id})")
            return order_id

        payload = {
            "token_id": token_id,
            "price": "0.0",
            "size": str(order_size_usd),
            "side": "buy",
            "type": "market",
        }
        r = requests.post(
...
async def run():
    ...
    # Historial reciente del midpoint de ETH, para poder chequear "¿ya
    # se movió ETH en los últimos 5 segundos, o sigue quieto?" -- si ya
    # se movió, la ventana de oportunidad se cerró y NO hay que operar,
    # aunque BTC haya saltado.
    eth_recent_history = deque()
    ETH_ALREADY_MOVED_THRESHOLD_CENTS = 1.0  # si ETH se movió más que esto, se considera que "ya reaccionó"
    state = {"market": market, "traded_this_window": False, "market_since": time.time()}
    ...
    async def refresh_market_if_needed():
        ...
        Tiene DOS chequeos independientes, para que no volvamos a
        quedar pegados en un mercado viejo por 17+ minutos como pasó:
        1. El normal, basado en end_ts (de la API o del propio slug).
        2. Un seguro por tiempo: si llevamos más de 6 minutos en el
           mismo mercado (más que una vela entera de 5min), fuerza la
           búsqueda igual, sin importar qué haya pasado con end_ts.
        """
        end_ts = state["market"].get("end_ts")
        window_expired = end_ts and time.time() >= end_ts
        safety_expired = (time.time() - state["market_since"]) > 6 * 60

        if window_expired or safety_expired:
            motivo = "ventana cerrada (end_ts)" if window_expired else "SEGURO por tiempo (6min+ en el mismo mercado, end_ts no disparó)"
            print(f"[MERCADO] Buscando la siguiente ventana -- motivo: {motivo}")
            try:
                ...
                if new_market["slug"] != state["market"]["slug"]:
                    ...
                    state["market_since"] = time.time()
                    print(f"[MERCADO] Nueva ventana: {new_market['slug']}")
                elif safety_expired:
                    # Mismo slug de vuelta pero ya pasaron 6+ min -- algo
                    # raro pasa (¿la API está devolviendo un mercado
                    # viejo?). Reseteamos igual el contador de tiempo
                    # para no spamear este chequeo cada 15s.
                    state["market_since"] = time.time()
                    print(f"[AVISO] find_active_eth_5m_market() devolvió el mismo "
                          f"slug de siempre pese al seguro de 6min -- revisar "
                          f"manualmente si esto persiste.")
    ...
    async def execute_trade(direction: str, btc_price: float):
        ...
        # ---- Chequeo que faltaba: ¿ETH YA se movió, o todavía está
        # quieto? Si ETH ya reaccionó al salto de BTC, la ventana de
        # oportunidad ya se cerró -- comprar ahora sería al precio ya
        # ajustado, sin ninguna ventaja. Solo operamos si ETH se movió
        # MENOS que el umbral en la misma ventana de tiempo.
        if len(eth_recent_history) >= 2:
            eth_oldest = eth_recent_history[0][1]
            eth_newest = eth_recent_history[-1][1]
            eth_moved_cents = abs(eth_newest - eth_oldest)
            if eth_moved_cents >= ETH_ALREADY_MOVED_THRESHOLD_CENTS:
                print(f"[SKIP] BTC saltó (dirección={direction}) pero ETH ya se "
                      f"movió {eth_moved_cents:.2f}c en los últimos "
                      f"{JUMP_WINDOW_SECONDS}s -- la ventana ya se cerró, no se opera.")
                return  # NO marca traded_this_window -- puede intentarse
                        # de nuevo si hay otro salto más adelante en la misma vela
        else:
            print(f"[SKIP] BTC saltó pero todavía no hay suficiente historial "
                  f"de ETH para confirmar si se movió o no -- se espera al "
                  f"próximo salto.")
            return
        ...
        # Orden de mercado real: precio 0.0 (Polymarket ejecuta al mejor
        # precio disponible, no hace falta calcularlo nosotros) y size =
        # el monto en dólares directo, sin dividir por nada -- mismo
        # patrón exacto que tu bot Flask que ya funciona
        # (send_order_with_retry(token_id, 0.0, order_size, "market")).
        print(f"[TRADE] Salto de BTC ({btc_price:.2f}, dirección={direction}) "
              f"+ ETH todavía quieto -> comprando {direction.upper()} en "
              f"{market['slug']} (orden de mercado, ${ORDER_SIZE_USD})")

        try:
            exchange.place_aggressive_order(token_id, ORDER_SIZE_USD)
        except Exception as e:
                    ...
                    async for raw in ws:
                        ...
                        for msg in messages:
                            ...
                            if midpoint is not None:
                                ...
                                # Guarda el historial para poder chequear
                                # "¿ETH ya se movió en los últimos 5s?"
                                # antes de decidir operar.
                                now = time.time()
                                eth_recent_history.append((now, midpoint))
                                cutoff = now - JUMP_WINDOW_SECONDS
                                while eth_recent_history and eth_recent_history[0][0] < cutoff:
                                    eth_recent_history.popleft()
                finally:
                    watcher.cancel()
            except Exception as e:
